...
| **Argo CD** | per-Application health + sync status, **OutOfSync/drift**, sync failures, reconcile latency, repo-server/redis up, **sync-operation + PostSync-hook failures** | Argo is the deploy control plane; **drift = an out-of-band change (governance/security concern) or a failed sync**. Prod now **auto-syncs with `selfHeal`** (REC-ADR-011) → alert on *sustained OutOfSync / repeated selfHeal corrections / sync-operation failures* (selfHeal silently reverting out-of-band tampering is itself a security signal), not on a "pending manual sync" state (which no longer exists). **A failing PostSync hook (the PBI-lifecycle callback, now an Argo PostSync hook — still to implement, #184) silently breaks the planning→delivery audit trail** → alert on PostSync-hook / sync-operation failure (§5.1). |
...
- **Reviewed where it touches prod:** the platform add-on auto-syncs (it's an invariant), and prod
  app sync is now auto too (REC-ADR-011), so the gate is **upstream** (PR + CI). Any alert-rule
  change that could *silence* a prod alert, and the off-cluster meta-monitor, must clear that same
  upstream PR-review gate before it reaches git. The Argo **PostSync hook (PBI-lifecycle callback,
...
| **D-M2** | CNPG exporter | **Add `monitoring.enablePodMonitor: true` to each `deploy/k3s/cnpg/cluster-*.yaml`** (currently absent). Highest-value one-line change — surfaces WAL-archive-to-fsn1 + PITR readiness. | ✅ **RESOLVED 2026-06-20 — YES** (founder); applied via a config PR. |
...
| **D-M9** | Meta-monitor home (the real SPOF call) | The `.166` docker-compose meta-monitor **shares `.166`'s failure domain** — a `.166` loss takes the cluster node, Forgejo, Infisical, and the watcher together (burst nodes are scale-to-zero, so unusable as a home). **Either (a) accept the residual `.166` SPOF** (external DMS is then the only guaranteed off-estate pager) **or (b) fund a tiny always-on external box** (≈ CX22 €4–5/mo) for a truly off-estate meta-monitor. | ✅ **RESOLVED 2026-06-20 — (a) accept the `.166` SPOF** (founder); the external dead-man's-switch is the only guaranteed off-estate pager. |
| **D-M10** | Surreal export RPO discrepancy | The sources **disagree**: CronJob is **hourly (`0 * * * *`)**, the design Decisions register says **≤24h (daily)**, the execution sequence says **~1h**. The alert is set to export-age **> ~70 min** (the CronJob reality), which **tightens the design's ≤24h RPO**. Founder must reconcile: either ratify the hourly RPO into the design, or relax the alert to match ≤24h. | ✅ **RESOLVED 2026-06-20 — RPO = 1h** (founder); ratify hourly into the design (the ≤24h figure is corrected), the >~70 min export-age alert stands. |
...
| **D-M13** | UI hostnames | ✅ **RESOLVED 2026-06-20 — per-UI subdomains under `*.allotmentology.tech`** (`grafana.`/`hubble.`/`argo.`/`alerts.`/`status.allotmentology.tech`), NOT `*.restormel.dev`. **Reason (D-M14 finding):** the portal's Better-Auth SSO cookie is scoped to `.allotmentology.tech`; cross-apex cookies don't flow, so hosting under the same apex lets the dashboards share the session directly. (Needs the `*.allotmentology.tech` wildcard cert.) | ✅ **RESOLVED — `*.allotmentology.tech`** |
| **D-M14** | Dashboard auth (portal SSO) | ✅ **RESOLVED 2026-06-20 — reuse the EXISTING portal forward-auth.** Investigation: the allotmentology.tech portal already runs SSO — a Traefik `forwardAuth` middleware → `https://allotmentology.tech/api/forward-auth` (Better-Auth session, cookie `.allotmentology.tech`), **proven in prod with Coolify** (the one `shared` launchpad tile; `web/infra/traefik/coolify-portal.deployed.yaml`). So **NO oauth2-proxy / no SSO rebuild** — port the same `forwardAuth` middleware into the **in-cluster K3s Traefik** (it calls the public portal endpoint outbound), host the dashboards under `*.allotmentology.tech` (D-M13) so the cookie flows, and the portal session carries straight in. Follow-ups: **update `launchpad.ts`** in the allotmentology repo (add Ops Centre/Grafana/Argo as `shared` tiles; **drop the stale Sentry** [never deployed] + **Coolify** [retiring→Argo]); **confirm/enable MFA on Better Auth** for the infra gate. Route the cluster-Traefik wiring through `restormel-high-risk-security`. | ✅ **RESOLVED — reuse forward-auth (no oauth2-proxy)** |
