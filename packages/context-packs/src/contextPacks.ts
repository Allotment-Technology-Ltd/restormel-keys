...
const PASS_TOKEN_BUDGET: Record<"quick" | "standard" | "deep", Record<ContextPackPass, number>> = {
...
function computeRoleCountsFromClaims(
  retrieval: ContextPackRetrievalInput,
  claimIndices: number[]
...
function selectForPass(
  ...
  const relationIndices = retrieval.relations
    ...
    .filter(
      ({ relation }) =>
        selectedIndexSet.has(relation.from_index) && selectedIndexSet.has(relation.to_index)
    )
    .sort(
      (a, b) =>
        relationPriority(pass, b.relation.relation_type) -
        relationPriority(pass, a.relation.relation_type)
    )
...
function replyAndTensionCounts(
  retrieval: ContextPackRetrievalInput,
  relationIndices: number[]
): { replyChains: number; unresolvedTensions: number } {
...
function buildEmptyPack(pass: ContextPackPass, tokenBudget: number): ContextPack {
  const emptyLine = "No knowledge base context available for this query.";
  return {
    pass,
    block: emptyLine,
    stats: {
      token_budget: tokenBudget,
      estimated_tokens: estimateTokens(emptyLine),
      truncated: false,
      claim_count: 0,
      relation_count: 0,
      argument_count: 0,
...
/**
 * Builds three pass-specific context pack blocks (analysis, critique, synthesis) from a portable
 * retrieval-shaped input. Token budgets follow SOPHIA `depthMode` tables; estimation uses `ceil(len/4)`.
 */
export function buildPassSpecificContextPacks(
  ...
  return {
    analysis: buildPackForPass("analysis", input, PASS_TOKEN_BUDGET[depthMode].analysis),
    critique: buildPackForPass("critique", input, PASS_TOKEN_BUDGET[depthMode].critique),
    synthesis: buildPackForPass("synthesis", input, PASS_TOKEN_BUDGET[depthMode].synthesis),
  };
}
