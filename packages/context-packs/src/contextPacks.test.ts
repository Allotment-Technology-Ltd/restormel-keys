...
import { buildPassSpecificContextPacks } from "./contextPacks.js";
import type { ContextPackRetrievalInput } from "./types.js";

/** Portable subset of the SOPHIA `contextPacks.test.ts` fixture (same graph, no Surreal-only fields). */
...
describe("buildPassSpecificContextPacks", () => {
  ...
  it("enforces token budget by truncating when budget is tiny", () => {
    ...
    expect(packs.analysis.stats.estimated_tokens).toBeLessThanOrEqual(packs.analysis.stats.token_budget);
  });
});
