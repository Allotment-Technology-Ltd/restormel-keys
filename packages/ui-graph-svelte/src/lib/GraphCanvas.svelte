<script lang="ts">
	...
	/** Measured container pixels; layout falls back to props until first measure. */
	let measuredWidth = $state(0);
	let measuredHeight = $state(0);

	const layoutWidth = $derived(Math.max(measuredWidth > 0 ? measuredWidth : width, 320));
	const layoutHeight = $derived(Math.max(measuredHeight > 0 ? measuredHeight : height, 240));
	...
	$effect(() => {
		...
		const nextPositions = computeLayout(nodes, edges, layoutWidth, layoutHeight);
		if (showGhostLayer && ghostNodes.length > 0) {
			...
			if (unanchored.length > 0) {
				...
				const radius = Math.min(layoutWidth, layoutHeight) * 0.42;
	...
	const detailPosition = $derived.by(() => {
		...
		return {
			x: Math.min(
				Math.max(screenX, (panelWidth / 2) + margin),
				layoutWidth - (panelWidth / 2) - margin
			),
			y: Math.min(Math.max(screenY, panelHeight + margin), layoutHeight - margin)
		};
	});
	...
	const edgeDetailPosition = $derived.by(() => {
		...
		return {
			x: Math.min(Math.max(midX, 160), layoutWidth - 160),
			y: Math.min(Math.max(midY, 64), layoutHeight - 64)
		};
	});
	...
	function fitToGraph(): void {
		...
		const padding = 36;
		const availableW = Math.max(layoutWidth - (padding * 2), 1);
		const availableH = Math.max(layoutHeight - (padding * 2), 1);
		const nextZoom = clamp(Math.min(availableW / graphW, availableH / graphH), MIN_ZOOM, MAX_ZOOM);
		zoom = nextZoom;
		panX = ((layoutWidth - graphW * zoom) / 2) - (minX * zoom);
		panY = ((layoutHeight - graphH * zoom) / 2) - (minY * zoom);
	}
	...
	function resetLayout(): void {
		positions = computeLayout(nodes, edges, layoutWidth, layoutHeight);
		setSelectedNodeId(null);
		highlightedNodeId = null;
		focusedNodeId = null;
		hasUserAdjustedView = false;
	...
	>
		...
		<rect width={layoutWidth} height={layoutHeight} fill="var(--color-bg)" />
...
<style>
	.graph-canvas-container {
		position: relative;
		width: 100%;
		/* Percentage height often resolves to 0 in flex/grid parents without a definite track height. */
		min-height: 400px;
		height: 100%;
		max-height: 100%;
		overflow: hidden;
