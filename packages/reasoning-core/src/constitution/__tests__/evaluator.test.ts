...
import { deterministicFixtures } from "./fixtures/deterministic-fixtures.js";

function createMockContext(generateTextImpl: ReasoningCoreContext["generateText"]): ReasoningCoreContext {
  return {
    generateText: generateTextImpl,
    resolveExtractionRoute: vi.fn(async () => ({
      model: "mock-extraction-model",
      modelId: "mock-extraction",
      provider: "mock",
    })),
    resolveReasoningRoute: vi.fn(async () => ({
...
describe("evaluateConstitution", () => {
  ...
  it("uses one LLM batch call for non-deterministic rules and merges all 10 rule outputs", async () => {
    ...
    const result = await evaluateConstitution(fixture.claims, fixture.relations, fixture.text, ctx);
