...
import type { IngestPlanningDeps } from '../ports.js';

const {
  mockResolveExtractionModelRoute,
  mockResolveReasoningModelRoute,
  mockBuildExtractionOpenAiCompatibleRoute,
  mockGetDefaultSharedRouteId,
  mockGetStoredRouteIdForStage
} = vi.hoisted(() => ({
  mockResolveExtractionModelRoute: vi.fn(),
  mockResolveReasoningModelRoute: vi.fn(),
  mockBuildExtractionOpenAiCompatibleRoute: vi.fn(),
  mockGetDefaultSharedRouteId: vi.fn(),
  mockGetStoredRouteIdForStage: vi.fn()
}));

function createTestDeps(overrides: Partial<IngestPlanningDeps> = {}): IngestPlanningDeps {
  return {
    getStoredRouteIdForStage: mockGetStoredRouteIdForStage,
    getDefaultSharedRouteId: mockGetDefaultSharedRouteId,
    getEmbeddingPlan: () => ({ name: 'vertex', model: 'text-embedding-005' }),
...
describe('planIngestionStage', () => {
  beforeEach(() => {
    ...
    mockGetDefaultSharedRouteId.mockResolvedValue(undefined);
    mockGetStoredRouteIdForStage.mockResolvedValue(undefined);
  ...
  it('forces EXTRACTION_* OpenAI-compatible route even with a bound route when INGEST_FORCE_EXTRACTION_OPENAI_COMPAT=1', async () => {
    ...
    mockGetStoredRouteIdForStage.mockResolvedValue('bound-route-id');
    mockBuildExtractionOpenAiCompatibleRoute.mockReturnValue({
      model: Symbol('ft-model'),
      provider: 'openai',
  ...
  it('uses Neon app AI default shared route when per-stage binding is absent', async () => {
    mockGetDefaultSharedRouteId.mockResolvedValue('app-default-shared-route');
    mockResolveReasoningModelRoute.mockResolvedValue({
      model: Symbol('rel'),
      provider: 'vertex',
      modelId: 'gemini-3-flash-preview',
  ...
  it('builds phase token/complexity estimates for pre-scan', async () => {
    const { buildIngestionStageUsageEstimates } = await import('../ingest/plan.js');
