...
Production and staging run on **Coolify** deployed to self-hosted Hetzner boxes (primary box `77.42.125.150`; second box `77.42.124.167` for SurrealDB and monitoring). **Forgejo** (`git.allotmentology.tech`) is the primary git host and CI runner; GitHub is a push-mirror and fallback. The operational database is a self-hosted Postgres instance on-box. Secrets and credentials are managed via Coolify environment variables and never committed to the repository.
