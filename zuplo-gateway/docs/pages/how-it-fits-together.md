...
| **Dashboard** | [restormel.dev/keys/dashboard](https://restormel.dev/keys/dashboard/) | Sign in, create projects, **create API keys** (`rk_...`). You get your backend key and manage projects here. |
...
2. **From the Dashboard** — After creating a project and API key, the dashboard can point you to the Cloud API docs (this portal) for how to call the API.
3. **Direct** — Bookmark or share this portal URL: `https://restormel-keys-gateway-main-bc13eba.zuplo.site`

## Flow in short

- **You (developer)** sign in at the **Dashboard** and create a project and an API key → that key is used by the **gateway** when it forwards requests to the dashboard backend.
