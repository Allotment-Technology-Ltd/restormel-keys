...
**Repo shape:** Monorepo (pnpm). `packages/` (core, svelte, elements, react, cli, doctor, validate, **aaif**, **mcp**), `platform/packages/tokens` (`@restormel/keys-tokens`), `apps/` (dashboard — single SvelteKit app for Keys landing, docs/walkthrough with optional agent prompts, **Integrations landing** at `/integrations`, and dashboard; demo-next, demo-svelte; site archived), `docs/`, `scripts/`, `prompts/`, `skills/`, `subagents/`. **New Restormel modules** (outside this repo): default stack in [docs/restormel-module-default-stack.md](docs/restormel-module-default-stack.md). **npm install scope** (headless vs UI, pnpm workspaces): [docs/reference/npm-packages.md](docs/reference/npm-packages.md).
...
**Design system:** All UI (site, dashboard, embeddable components, demos) aligns with [docs/design-system-index.md](docs/design-system-index.md). Tokens and components from DESIGN-TOKENS.md, DESIGN-SPECIFICATION.md, COMPONENT-INVENTORY.md; reference implementation in docs/design-tokens.css. Shared token package: [platform/packages/tokens](platform/packages/tokens) (`@restormel/keys-tokens`, base + semantic rm/rk); drift check: `pnpm run check-token-drift`. **Platform subtree:** [platform/](platform/) holds tokens, reusable CI composites, and Cursor template for suite modularisation; see [docs/platform-modularization.md](docs/platform-modularization.md).
...
- **Tokens:** Base canonical tokens → semantic surface tokens (`--rm-*` brand/app/docs, `--rk-*` embed) → optional component tokens. See [docs/design-system-index.md](docs/design-system-index.md) and [platform/packages/tokens](platform/packages/tokens).
