<script lang="ts">
  ...
  async function createGatewayKey() {
    ...
    newKey = null;
    try {
      const res = await fetch(`${base}/api/projects/${selectedProjectId}/keys`, { method: "POST" });
  ...
  async function revokeKey(key: KeyWithProject) {
    ...
    revokingId = key.id;
    try {
      const res = await fetch(`${base}/api/projects/${key.projectId}/keys`, {
        method: "DELETE",
...
{:else}
    ...
    {#if data.keys.length === 0 && !newKey}
      ...
      >
        {#if data.projects.length === 0}
          <a href={base + "/projects"} class="btn btn-primary">Create a project first</a>
        {:else}
  ...
  <section class="section" aria-labelledby="audit-heading">
    ...
    <a href={base + "/access/audit"} class="btn btn-secondary">View audit log</a>
  </section>
{/if}
