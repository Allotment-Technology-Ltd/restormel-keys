...
{:else if data.error}
  ...
  <p><a href={base + "/logs"}>View Logs & Traces</a> for raw request data.</p>
{:else if data.aggregates.length === 0 && data.recentLogs.length === 0}
  <div class="empty-state" role="status">
    ...
    <a href={base + "/routes"} class="btn btn-primary">Routes</a>
    <a href={base + "/logs"} class="btn btn-secondary">Logs & Traces</a>
  </div>
{:else}
        ...
        {:else}
          <ul class="mix-list">
            {#each modelMix() as m}
              <li>
                {#if m.name !== "—"}
                  <a href={base + "/models/" + encodeURIComponent(m.name)} class="mix-link">{m.name}</a>
                {:else}
            ...
            {#each routeMix() as m}
              <li class="mix-list-item-route">
                <span class="mix-route-primary">
                  {#if m.routeId}
                    <a href={m.projectId ? base + "/projects/" + m.projectId + "/routes/" + m.routeId : "#"} class="mix-link">{m.routeId.slice(0, 8)}…</a>
                  {:else}
                    <span class="mix-name">—</span>
                  {/if}
                  <span class="mix-count">{m.count.toLocaleString()}</span>
                ...
                {#if m.routeId && m.projectId}
                  <a href={base + "/logs?routeId=" + encodeURIComponent(m.routeId) + "&projectId=" + encodeURIComponent(m.projectId)} class="mix-sublink">Logs</a>
                {/if}
              </li>
            {/each}
          </ul>
  ...
  <section class="section" aria-labelledby="recent-heading">
    ...
    <p class="section-desc">
      Latest request logs in the period. For full list and filters, use <a href={base + "/logs"}>Logs & Traces</a>.
    </p>
    ...
    {:else}
      <ul class="log-list">
        {#each data.recentLogs as log}
          <li class="log-row">
            ...
            {#if log.routeId}
              <a href={base + "/projects/" + log.projectId + "/routes/" + log.routeId} class="log-route">Route</a>
            {:else}
      ...
      <p><a href={base + "/logs"} class="btn btn-secondary">Open Logs & Traces</a></p>
    {/if}
  </section>
{/if}
