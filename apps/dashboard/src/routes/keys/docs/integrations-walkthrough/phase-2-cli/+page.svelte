...
<svelte:head>
  ...
  <meta name="description" content="Install the CLI, run doctor, validate, models list, routing explain." />
</svelte:head>
...
<div class="doc-content">
  ...
  <p>This phase installs the Restormel Keys CLI and runs doctor, validate, models list, and routing explain so you can debug and inspect from the terminal.</p>
  ...
  <p>Then run via <code>npx keys</code> or <code>pnpm exec keys</code>. You'll see <code>keys --help</code> with init, add, list, validate, doctor, estimate, sync, models, routing.</p>

  <h2>Step 2.2 — Run doctor</h2>
  ...
  <h2>Step 2.3 — Validate (optional)</h2>
  ...
  <h2>Step 2.5 — Routing explain</h2>
  ...
  <AgentPromptsSection
    intro="Optional. Use if a coding agent should install the CLI and verify doctor, models list, and routing explain."
    prompts={agentPrompts}
    defaultOpen={false}
  />

  <h2>Checkpoint</h2>
  <p>You now have: CLI installed (global or dev dependency), doctor passing, <code>keys models list</code> and <code>keys routing explain</code> working, and optionally validate run for local credentials.</p>
