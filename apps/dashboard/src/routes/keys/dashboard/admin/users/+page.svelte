<script lang="ts">
  ...
  export let data: { adminUsers: AdminUserListRow[] };
  ...
  async function toggleDbOperator(row: AdminUserListRow, next: boolean) {
    ...
    savingId = row.id;
    try {
      const res = await fetch(`${DASHBOARD_BASE}/api/admin/users/${encodeURIComponent(row.id)}`, {
        method: "PATCH",
...
<p class="page-foot">
  <a class="link" href={DASHBOARD_BASE + "/settings"}>Back to settings</a>
</p>
