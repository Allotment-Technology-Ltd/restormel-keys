<script lang="ts">
  ...
  export let data: {
    ...
    /** Relocated Connect-hub home panels (R2) — R3 mounts these properly. */
    hubSignedIn: boolean;
  ...
  $: connectSteps = [
    {
      ...
      href: INGEST_FLOW_HREF + "?step=store",
  ...
  {#if scorecard}
    ...
    <div class="trust-strip panel" aria-label="Verified context status">
      ...
      <div class="trust-strip-ctas">
        <a class="trust-cta" href="#trust-ledger">View trust ledger</a>
        {#if needsReview > 0}
          <a class="trust-cta trust-cta--review" href={CLAIMS_HREF + "?filter=review"}>
    ...
    <div class="trust-strip trust-strip--empty panel" aria-label="No verified context yet">
      <div class="trust-strip-inner">
        <p class="trust-empty-msg">
          ...
          <a href={INGEST_FLOW_HREF + "?step=store"}>Start your first ingest run</a>
  ...
  {#if allConnectDone}
    <div class="next-step-card next-step-card--done">
      ...
      <p class="next-step-desc">
        ...
        <a href={CLAIMS_HREF}>Explore the graph</a>
        or
        <a href={RUNS_HREF}>start a new run</a>.
      </p>
    </div>
...
<!-- ── Connect workspace panels ─────────────────────────────────────────── -->
<!-- Relocated mechanically from the dissolved Connect hub home (R2 §2.3:
     /connect MERGE-INTO /home). R3 ("One Home") mounts these properly as the
     masthead; until then they keep the hub's exact behaviour. -->
{#if data.hubSignedIn}
  <section class="connect-workspace" id="trust-ledger" aria-labelledby="connect-workspace-h">
    <h2 id="connect-workspace-h" class="overview-section-title">Verified context workspace</h2>
    ...
    {:then hub}
      {#if !hub?.journey}
        ...
        <BrutalErrorBanner
          title="Connect workspace unavailable"
      ...
      {:else}
        ...
        {@const connectNext = journey.steps.find((s) => s.id === journey.nextStepId) ?? null}
...
<style>
  ...
  /* ── Relocated Connect-hub panels (R2) ─────────────────────────────── */
  .connect-workspace {
