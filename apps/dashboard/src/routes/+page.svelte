  ...
  <section class="onboarding-section" aria-labelledby="onboarding-heading">
    ...
    <ol class="onboarding-steps" aria-label="Onboarding steps">
      ...
      <li class="step" class:step-done={o?.hasProjects}>
        ...
        {:else}
          <a href={base + "/projects"} class="step-action">Create a project</a>
        {/if}
      </li>
      ...
      <li class="step step-info"><span class="step-label">Billing mode</span> — Set per route: bring your own keys or Restormel-managed. <a href={base + "/billing"} class="step-action">Billing</a></li>
      <li class="step" class:step-done={o?.hasKeys}>
        {#if o?.hasKeys}
          <span class="step-label">Gateway Key</span> — Done.
        {:else}
          <a href={base + "/access"} class="step-action">Create a Gateway Key</a> (Access)
        {/if}
      </li>
      <li class="step" class:step-done={o?.hasIntegrations}>
        ...
        {:else}
          <a href={base + "/integrations"} class="step-action">Connect a provider</a> (Integrations)
        {/if}
      </li>
      <li class="step">
        <a href={base + "/routes"} class="step-action">Create a route</a> — Choose models and fallbacks.
      </li>
      <li class="step">
        <a href="/keys/docs/" class="step-action" target="_blank" rel="noopener noreferrer">First request</a> — Then check <a href={base + "/logs"}>Logs</a>.
      </li>
      <li class="step">
        <a href={base + "/analytics"} class="step-action">Analytics</a> — Request count, latency, usage by provider and route.
      </li>
    </ol>
    {#if o && !o.hasProjects}
      <p class="onboarding-cta">
        <a href={base + "/projects"} class="btn btn-primary">Create your first project</a>
      </p>
    {/if}
  </section>
{/if}
