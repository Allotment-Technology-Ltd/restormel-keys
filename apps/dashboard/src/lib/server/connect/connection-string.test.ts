...
describe("parseSurrealConnectionString", () => {
  it("parses a full wss string with credentials and ns/db (scheme preserved)", () => {
    const r = parseSurrealConnectionString("wss://ingest:s3cret@db.surreal.cloud/restormel/knowledge");
    expect(r.error).toBeUndefined();
    expect(r.endpoint).toBe("wss://db.surreal.cloud");
  ...
  it("preserves ws scheme and keeps the port", () => {
    const r = parseSurrealConnectionString("ws://localhost:8000/ns/db");
    expect(r.endpoint).toBe("ws://localhost:8000");
  ...
  it("maps the surrealdb:// alias to https (no wire scheme of its own)", () => {
  ...
  it("parses ADO-style .NET connection strings (wss preserved)", () => {
    ...
    expect(r.error).toBeUndefined();
    expect(r.endpoint).toBe("wss://abc123.aws-euw1.surreal.cloud");
  ...
  it("parses ADO aliases (Endpoint, NS, DB, User, Pass; wss preserved)", () => {
    const r = parseSurrealConnectionString(
      "Endpoint=wss://cloud.example.surreal.cloud;NS=team;DB=graph;User=ingest;Pass=token",
    );
    expect(r.endpoint).toBe("wss://cloud.example.surreal.cloud");
    expect(r.namespace).toBe("team");
    expect(r.database).toBe("graph");
    expect(r.username).toBe("ingest");
  ...
  it("ignores /rpc path segment when extracting namespace/database", () => {
    ...
    expect(r.endpoint).toBe("wss://instance.surreal.cloud");
    expect(r.namespace).toBeUndefined();
    expect(r.database).toBeUndefined();
  });
  ...
  it("parses passwords containing @ via manual URI parsing (scheme preserved)", () => {
    const r = parseSurrealConnectionString("wss://root:p@ss@word@host.surreal.cloud/ns/db");
    expect(r.username).toBe("root");
    expect(r.secret).toBe("p@ss@word");
    expect(r.endpoint).toBe("wss://host.surreal.cloud");
    expect(r.namespace).toBe("ns");
    expect(r.database).toBe("db");
  });

  it("strips wrapping quotes (scheme preserved)", () => {
  ...
  it("parses labelled multi-line paste (scheme preserved)", () => {
...
Password: secret`);
  ...
  it("parses surreal CLI snippets (scheme preserved)", () => {
    const r = parseSurrealConnectionString(
  ...
  it("parses Surreal Cloud CLI copy (endpoint + token only; scheme preserved)", () => {
    ...
    expect(r.error).toBeUndefined();
    expect(r.endpoint).toBe("wss://sophia-prod-06ejjp0ud1ql95k42iqg8esvhs.aws-euw1.surreal.cloud");
    expect(r.secret).toBe(token);
  ...
  it("preserves an explicit https scheme", () => {
    const r = parseSurrealConnectionString("https://instance.surreal.cloud/ns/db");
