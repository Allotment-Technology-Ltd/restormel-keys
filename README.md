...
|---------|---------|-------------|
| [@restormel/keys](packages/core) | 0.2.1 | Headless core: routing, cost, providers (OpenAI, Anthropic, Google), storage, server middleware |
...
| [@restormel/doctor](packages/doctor) | 0.1.1 | OSS CLI: setup and health checks (`restormel-doctor`) |
| [@restormel/validate](packages/validate) | 0.1.1 | OSS CLI: credential/config validation (`restormel-validate`) |
...
To publish all packages: ensure each has `version`, `files`, README, and `pnpm run build`; run `pnpm -r publish --dry-run` from repo root; then tag `keys-v0.2.1` (or next `keys-v*`) and push to trigger the publish workflow. Do not publish the Vue wrapper. Test files are not included in `files` and stay in the repo.
