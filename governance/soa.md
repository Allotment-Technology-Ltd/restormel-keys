...
ISO 27001:2022 Annex A — all 93 controls. Scope: **Restormel** (restormel.dev, Keys + Connect
products). Organisation: Allotment Technology Ltd (sole director). ICO registration: ZC092549.
...
| A.5.14 | Information transfer | Partial | TLS enforced for all data in transit. DPAs in place for sub-processors. UK→US transfer safeguards partially confirmed; Google Workspace and Zuplo DPAs outstanding (RISK-003). | Partial | REC-GOV-005; RISK-003 |
| A.5.15 | Access control | Yes | Access control policy approved | Implemented | REC-POL-002 |
| A.5.16 | Identity management | Yes | BetterAuth (self-hosted on AST-003); named accounts only; no shared logins | Implemented | REC-POL-002; AST-003 |
| A.5.17 | Authentication information | Partial | MFA required per REC-POL-002 §3; enablement in progress across all systems (RISK-008); remediation task scheduled. | Partial | REC-POL-002 §3; RISK-008 |
...
| A.5.20 | Addressing IS within supplier agreements | Partial | DPAs accepted (standard terms) for most suppliers; two require explicit in-portal acceptance (Hetzner AVV, Google Workspace DPA) — outstanding action items A1 and A2. | Partial | REC-GOV-005; RISK-003 |
...
| A.5.23 | Information security for use of cloud services | Yes | Cloud services documented in REC-GOV-005 with DPA status, data location, and sovereignty flag; sovereignty policy in REC-POL-001 §4 | Implemented | REC-GOV-005; REC-POL-001 |
...
| A.5.31 | Legal, statutory, regulatory and contractual requirements | Yes | UK GDPR compliance: RoPA (REC-GOV-003), DPAs (REC-GOV-005), privacy notice (restormel.dev/keys/privacy), data-inventory (REC-GOV-007). ICO registration: ZC092549. Companies Act: statutory records (FreeAgent/Mettle, 7-year retention). | Implemented | REC-GOV-003; REC-GOV-007; REC-GOV-005 |
...
| A.5.34 | Privacy and protection of PII | Yes | UK GDPR compliance: RoPA (REC-GOV-003), data-inventory (REC-GOV-007), privacy notice published, DPAs in REC-GOV-005 | Implemented | REC-GOV-003; REC-GOV-007; restormel.dev/keys/privacy |
...
| Control | Title | Applicable | Justification / Notes | Status | Evidence |
|---|---|---|---|---|---|
| A.7.1 | Physical security perimeters | N/A — justified | No company office or data centre. Compute hosted on Hetzner Helsinki (physical perimeters managed and certified by Hetzner). Home office is sole physical location — standard residential security applies. | N/A — justified | REC-GOV-005 (Hetzner); AST-003 |
...
| A.8.3 | Information access restriction | Yes | BetterAuth (self-hosted) for product authentication; API keys scoped per project (rk_* gateway keys); no guest/anonymous access to sensitive functions. | Implemented | REC-POL-002; AST-004 |
| A.8.4 | Access to source code | Yes | Private Forgejo repos; founder-only write access; GitHub is read/push-only mirror; branch protection on main enforced by CI; no external contributor access. | Implemented | AST-002 (Forgejo); CI branch protection |
| A.8.5 | Secure authentication | Partial | BetterAuth (self-hosted) in production; MFA requirement defined in REC-POL-002 §3; enablement in progress across all systems (RISK-008). Remediation task scheduled. | Partial | REC-POL-002 §3; RISK-008 |
...
| A.8.13 | Information backup | Yes | Hetzner automated backups for self-hosted Postgres (AST-003); Forgejo repo mirrored to GitHub (AST-002); governance records in Forgejo with 6-year retention. Recovery objective in RISK-001. | Implemented | AST-003 Hetzner backups; RISK-001; AST-002 |
...
| A.8.24 | Use of cryptography | Yes | TLS 1.2+ for all data in transit; FileVault AES-256 at rest (MacBook); Hetzner encrypted storage; BYOK model — provider credentials stored as ciphertext only; key management per REC-POL-001 §4. | Implemented | REC-POL-001 §4; AST-001; BYOK architecture |
...
| A.8.31 | Separation of dev, test and production environments | Partial | Forgejo CI has distinct build / test / deploy stages; no separate staging infrastructure (planned). Production Coolify instance also used for staging. RISK-001. | Partial | CI pipeline; RISK-001 |
