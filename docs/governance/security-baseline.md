...
**Keys routing metadata:** Step JSON such as **`model_pool`**, **`switchCriteria`**, and parallel hints are **control-plane intent** only. **Do not** store provider secrets or raw API keys in these blobs; resolve/simulate responses must not echo secrets ([keys-routing-contract.md](../architecture/keys-routing-contract.md)). **Hosted runtime** evaluates **`advanceOn`** only against an allowlisted subset for **failure advance** decisions; do not extend with unreviewed expression languages ([rfc/keys-no-code-route-runtime.md](../archive/deferred-products/hosted-runtime/keys-no-code-route-runtime.md) Phase 3).
