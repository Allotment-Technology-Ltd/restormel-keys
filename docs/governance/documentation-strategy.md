...
- **Progressive disclosure tiers:** Tier 0 = suite hub `/docs`; Tier 1 = quickstart + operator model + slim product sidebars; Tier 2 = walkthroughs, vendor guides, search — collapsed **Reference** sections in sidebars. Inventory: [docs/architecture/SUITE-IA-REDIRECT-INVENTORY.md](../architecture/SUITE-IA-REDIRECT-INVENTORY.md).
- **Canonical operator vocabulary:** [docs/product/SUITE-OPERATOR-MODEL.md](../product/SUITE-OPERATOR-MODEL.md) (mirrored at `/docs/operator-model`).
...
| Horizon platform programme (suite expansion) | `docs/architecture/HORIZON-PLATFORM-PROGRAMME.md` | Capability shortlist (A–J), Theme L IA + dashboard shell rules, Theme M pointer, MCP/AAIF inventory and backlog, canonical boundaries—does not replace topic-specific specs. |
| Horizon platform programme (suite expansion) | `docs/architecture/HORIZON-PLATFORM-PROGRAMME.md` | Capability shortlist (A–J), Theme L IA + dashboard shell rules, Theme M pointer, MCP/AAIF inventory and backlog, canonical boundaries—does not replace topic-specific specs. |
| Suite architecture migration (Keys / Graph / Knowledge) | `docs/architecture/SUITE-ARCHITECTURE-MIGRATION.md`, `docs/product/CONNECT-EXTRACTION-MAP.md`, `docs/archive/suite-migration-status/PHASE0-SUITE-MIGRATION-STATUS.md` | Phased delivery-model migration, SOPHIA extract + reintegrate playbook, stage gates; **Testing delivery changes post-MVP (out of scope)**. |
| Restormel Connect (product) | `docs/product/CONNECT-PRODUCT.md`, `@restormel/contracts/connect`, `docs/api/openapi-suite-v1-draft.yaml` | Fourth suite product definition and REST contract epoch. |
| Restormel Support (Theme M product) | `docs/architecture/RESTORMEL-SUPPORT.md`, `docs/runbooks/restormel-support-production.md` | In-product assistant: trust boundaries, production env, dogfood checklist. Package: `@restormel/support`. |
| Design system       | `docs/design/design-system-index.md`, `docs/design/design-tokens.css` | Tokens, brand, components. |
...
- When adding runbooks or reference docs that point to the product, add them to [runbooks.md](../runbooks/README.md) or the master index as appropriate and keep same-link compliance.
