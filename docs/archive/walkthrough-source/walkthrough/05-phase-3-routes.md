...
**Canonical routing contract (SOPHIA-class):** Resolve returns a **`stepChain`** (ordered tiers), optional **`fallbackCandidates`**, and a **`contractVersion`** (currently **`2026-04-16`**) with per-step metadata such as **`timeoutMs`**, **`retryPolicy`**, **`costPolicy`**, **`switchCriteria`**, optional **`model_pool`**, and parallel hints. Ingestion pipelines can attach **`workload: "ingestion"`** and a **`stage`** (for example `ingestion_embedding`) on the route so resolve discovers the right rule without baking in internal UUIDs; use **`attemptNumber`** plus **`previousFailure`** so Keys advances past exhausted tiers. Full semantics: [docs/keys-routing-contract.md](../keys-routing-contract.md) — in-product mirror: [/keys/docs/guides/routing-contract](https://restormel.dev/keys/docs/guides/routing-contract).
