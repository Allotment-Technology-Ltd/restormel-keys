...
| `@restormel/state` | **Restormel State** — append-only agent memory events, deterministic `projectWorkingMemory`, correlation with context packs and observability runs (depends on **`@restormel/context-packs`** for input typing). SOPHIA/Stoa-specific builders are documented in-repo for SOPHIA only, not exported from npm. |
