...
| **4. Neon Auth + GitHub** | In Neon Console (Auth → OAuth providers), add **GitHub**. Create a [GitHub OAuth App](https://github.com/settings/developers) and set **Authorization callback URL** to match Neon Auth for that branch: `{NEON_AUTH_BASE_URL}/callback/github` (see Neon Console → Auth). For the dashboard proxy, also register `https://restormel.dev/keys/dashboard/api/auth/callback/github` and, for local dev, `http://localhost:5173/keys/dashboard/api/auth/callback/github`. Each Neon branch has a different Auth host — add **every** branch callback URL you use. Enter the GitHub **Client ID** and **Client Secret** in Neon Console (not in app env). |
