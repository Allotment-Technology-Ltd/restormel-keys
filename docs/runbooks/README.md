...
| [runbooks/sso-consolidation-apply.md](sso-consolidation-apply.md) | **Estate SSO consolidation — APPLY runbook** (AUTH, high lockout risk; founder-present). Forgejo→portal OIDC, Grafana auth-proxy (already live), Argo OIDC, Infisical decision, and the `argocd.restormel.dev` decommission. Per-step commands + rollback + fallback-login proof; safe-now vs founder-present split. PRs: allotmentology.tech#13, restormel-gitops#28. |
| [runbooks/sso-saas-google-consolidation.md](sso-saas-google-consolidation.md) | **SaaS → Google login** (Step 3, founder actions only): exact per-vendor steps (GitHub, Notion, Vercel, PostHog) + keep financial/banking vendors independent. |
