---
title: Routing and catalog signals (design note)
class: technical
owner: founder
status: approved
...
- **No secrets** in telemetry or agent-facing summaries ([security baseline](../governance/security-baseline.md)).
- Correlate only **masked** identifiers and **aggregated** counts.

## Canonical contract

See [keys-routing-contract.md](../architecture/keys-routing-contract.md) for the authoritative routing API. This document reserves space for a future “signals → policy hints” design.
...
2. **Threat model sign-off** for poisoning, Sybil reports, and mis-scoped aggregates — see [threat-model-starter.md](../governance/threat-model-starter.md) (*Catalog observations → routing automation*).
