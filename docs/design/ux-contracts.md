...
Shared nav schema: site nav, docs sidebar, and dashboard sidebar use the same canonical URLs above. See [documentation-strategy.md](../governance/documentation-strategy.md) and [design-system-index.md](design-system-index.md).
...
> [`verified-context-claims-ledger.md`](../product/verified-context-claims-ledger.md). Stage W4.5 will
...
| **Operator verdict** (Supported / Weak / Unsupported / Pending) | The human-review verdict applied to a claim via `performReview` (internal status `ok` / `weak` / `unsupported` / unchecked). The canonical display labels are **Supported** (`ok`), **Weak** (`weak`), **Unsupported** (`unsupported`), **Pending** (unchecked) — the `GRAPH_REVIEW_VERDICT_VISUAL` legend in `lib/connect/graph-review-verdict-visual.ts` is the single source. **"Rejected" is retired** — the Stamping Desk's X stamp, its announcement, and its session tally all say "Unsupported" now (W4.5 duality resolution; the desk was the only surface that diverged). This is a separate vocabulary from the EBV verification states (supported / inferred / unverified / contradicted / excluded), which describe evidence binding, not a human verdict. | W4.5 |
...
| **Live-run chip**    | The topbar `LiveRunChip` (`● INGEST 62% · 2:41`) shown while an ingest run is active; links to `/runs/[id]`; amber `STALLED` when the worker heartbeat is stale (W1.4 model). Not "run pill" or "progress badge". Its `aria-label` spells out the state. State contract: §3 shell-element block. (Citation added W4.5 for the chip noun.) | R6, W4.5 |
| **Run quick-peek**   | The `RunQuickPeek` consumer of `DossierRail` opened from a Runs-list row — a read-only run summary. The control's accessible name is "Quick-peek this run" (verb "Quick-peek"). Not "preview" or "inspect". (Citation added W4.5.) | R6, W4.5 |
| **This run's audit** | The label on the run-console completion ledger's verdict cap (`/runs/[id]`). It distinguishes a *single run's* quality report from the standing **Trust scorecard** (which it QUOTES, never recomputes — W2.3 single-source rule). Not "run score" or "trust score" unqualified. (Citation added W4.5.) | W4.1, W4.5 |
...
| **Run console — completion ledger** (`/runs/[id]`, completed) | rides the run-console load | numbers not reported → the verdict cap reads **"—" / "Recorded"** (no fabricated score); supported reads `— supported` when no `ok_pct` was reported (muted, never a fabricated `0%`) and `0%` only when genuinely 0; the unit stat is **dropped entirely** when the run reported no count | n/a — display-only | **ONE** ledger replaces the old stacked success-banner + scorecard + "what to do next" blocks (deleted, not hidden): a neon verdict cap (trust numeral + verdict word + `N% supported · M units captured`, labelled **"This run's audit"**) above a single next-actions body. The trust/supported numbers QUOTE the run's `quality_report` (W2.3 single-source rule), reusing the scorecard's `trustScoreDescriptor` / `unitsSupportedDescriptor` thresholds (no forked cut-offs); **`M units captured` is the real captured unit count from `quality_report.units`**, NOT the completed-stage count (`progress.processed`, = pipeline stage length); the K4/K5 validating-family disclosure rides the cap; cross-links to the standing scorecard (`/home#trust-ledger`) and all runs are preserved |
...
| **Stamp bar** (`.desk-stamps`, `role="group"`) | n/a | n/a | **Supported (S) guarded off** for an unbound / pre-binding / no-evidence claim — disabled stamp + verbatim `canAcceptAsSupported` reason (`.desk-stamp-guard`), never a silent no-op (claims-ledger row 2) | S=Supported · W=Weak · X=Unsupported (the canonical operator-verdict label — W4.5 duality resolution, aligned with the explorer legend), each ≥44px, the AI-suggested verdict emphasised; a 100ms mechanical press on stamp (`.desk-stamp-flash`, `prefers-reduced-motion`-guarded) |
| **Session tally rail** (`.desk-tally`, `role="status"`) | n/a | "REVIEWED 0 · SUPPORTED 0 · WEAK 0 · UNSUPPORTED 0" | n/a | Live ledger line "REVIEWED N · SUPPORTED N · WEAK N · UNSUPPORTED N"; resets per visit (lives in desk state); an honest note that the **trust score** recomputes on the Home scorecard — the desk does not fork or fabricate a per-session score delta |
...
When adding or changing copy or nav, check this document and [documentation-strategy.md](../governance/documentation-strategy.md) for consistency.
...
**Source:** [`docs/design/keys-northstar-redesign-2026-06.md`](keys-northstar-redesign-2026-06.md) — all ten product decisions D1–D10 approved 2026-06-12. This re-baseline records those decisions as contracts and provides the redirect map R2 implements.
...
- **Roadmap amended** (in `docs/design/dashboard-world-class-roadmap.md`): re-scope notes for K4
