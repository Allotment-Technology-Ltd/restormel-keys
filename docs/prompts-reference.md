...
**Cursor Agent Skills vs prompts:** Repeated agent workflows live in **`.cursor/skills/<name>/SKILL.md`** (mirrored under `.agents/skills/`). Do not copy skill bodies into `prompts/reference` packs; **link** the skill or the canonical doc (for Keys routing: `docs/keys-routing-contract.md`). If a skill and a prompt pack would say the same operational truth, keep **one** authority per [prompt-governance.md](prompt-governance.md).
