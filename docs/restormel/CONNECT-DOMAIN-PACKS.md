...
**Related:** [CONNECT-PRODUCT.md](./CONNECT-PRODUCT.md), [CONNECT-EXTRACTION-MAP.md](./CONNECT-EXTRACTION-MAP.md), [SUITE-ARCHITECTURE-MIGRATION.md](./SUITE-ARCHITECTURE-MIGRATION.md), [Connect first graph onboarding](../connect-first-graph-onboarding.md)
...
- Graph store credentials are **AES-256-GCM encrypted at rest** (`RESTORMEL_CREDENTIALS_ENCRYPTION_KEY`); the secret is write-only and never returned by the API/UI. See [security-baseline.md](../security-baseline.md).
