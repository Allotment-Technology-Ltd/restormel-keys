...
| **Restormel Support Agent (Theme M, draft)** | Help / “Ask Restormel” entry (signed-in); future marketplace module narrative | Optional doc page when shipped; until then pointers from Dev Tools / docs search | **Site-wide** signed-in shell (root layout) + server route; not a second nav tree | N/A | Orchestrates existing **Read** tools (`docs.search`, `testing.journey`, …); avoid parallel tool names—see [HORIZON-PLATFORM-PROGRAMME.md §5](./HORIZON-PLATFORM-PROGRAMME.md) |
