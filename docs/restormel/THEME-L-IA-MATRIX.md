...
|--------|-------------------|-------------|---------------------|-------|
| **Restormel suite** | `https://restormel.dev/` — Run vs Embed hero; [`SiteHeader`](../../apps/dashboard/src/lib/components/site/SiteHeader.svelte) **Product · Integrations · Company · Developers** (not four product dropdowns) | **`https://restormel.dev/docs`** — Tier 0 hub; [`suite/docs-nav.ts`](../../apps/dashboard/src/lib/suite/docs-nav.ts) | Same `/keys/dashboard` shell; overview **capability chips** | Operator model: [SUITE-OPERATOR-MODEL.md](./SUITE-OPERATOR-MODEL.md) |
