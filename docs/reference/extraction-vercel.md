...
2. **Build** — In Vercel, set **Root Directory** to **`apps/dashboard`**. The repo’s `vercel.json` in that directory sets `installCommand` to run from repo root and `buildCommand` to `pnpm run build`. No need to override Output Directory; the adapter emits the correct structure.
