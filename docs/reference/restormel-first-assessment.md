...
| **Component behavior** | Packaged UI components behave like low-level shells. Host apps still wrap: loading states, degraded/error states, empty states, request-scoped routing, current selection display, retry handling. Affects all consumers. |
| **Model filtering** | Restormel still requires host app to own a custom allowed-models proxy and custom BYOK server flows. Acceptable as advanced path; Restormel is not yet doing enough for the **common** integration case. |
