...
| 4 | "Unsupported claims are excluded, not blended" | Remediation fail-safe: an omitted remediation verdict defaults to `drop` (reversible soft-exclude), never `keep`; orchestrator soft-excludes rather than hard-deletes. Retrieval-side: `connect.retrieve_verified` in strict mode returns ONLY `supported` claims — `excluded`, `contradicted`, `inferred`, and `unverified` claims are omitted, never silently included. | `stages.test.ts` — "fails omitted units safe as drop (not keep)"; repro C2 section. `pnpm --filter @restormel/mcp exec vitest run src/connect-verified-retrieval.test.ts` — "strict mode: excluded/contradicted/inferred/unverified claims are absent (row 4 proof)" + "strict mode: only supported claims are returned". | **proven** |
...
| 6 | "The validator catches fabricated claims" | Fabricated-tier recall ≥ 95% (signed-off bar). Measured 100% ± 0 across both pairings and both paths (legacy + EBV), 3 runs, 2026-06-10. Stays proven only while re-measured as models change. | **Continuous (Stage 2.3):** weekly cross-model re-run — [`connect-efficacy-weekly.yml`](../.github/workflows/connect-efficacy-weekly.yml) runs the Stage 1.0a harness (`--with-extraction --ebv --runs 3`, OpenAI extractor × Together validator) and [`check-efficacy-bars.mjs`](../scripts/ci/check-efficacy-bars.mjs) **fails the run if any signed-off bar regresses**; each run uploads a dated snapshot artifact + bar table. Current committed measurement: [`verifier-efficacy-results-2026-06-10-ebv.json`](../scripts/reviews/verifier-efficacy-results-2026-06-10-ebv.json). Local re-run: `pnpm exec tsx scripts/reviews/verifier-efficacy.ts --validator … --ebv --runs 3` (keys required). | **proven** — continuously enforced by the weekly run (red run ⇒ flip to **broken**, rule 3) |
...
| 8 | "Published quality bar: ≥90% supported, ≤2% unsupported" | `assertG2Targets` enforces ok_pct ≥ 90 and unsupported_pct ≤ 2 (`G2_OK_PCT_TARGET`/`G2_UNSUPPORTED_PCT_MAX`); `keys connect eval` returns exit 1 when a graph misses the bar. | `pnpm --filter @restormel/connect-core exec vitest run src/__tests__/golden-eval.test.ts`; headless: `keys connect eval --json` (Stage 2.1, PR #197). Per-PR CI gate (Stage 2.3): [`connect-eval-gate.yml`](../.github/workflows/connect-eval-gate.yml) proves the 0/1/2/3 exit contract on every Connect-pipeline PR (warn mode; live-ingest blocking flip condition in the workflow header). | **proven** |
