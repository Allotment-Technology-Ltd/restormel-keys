...
2. **The rich catalogue** — `apps/dashboard/data/model-catalog-seed.json` (143 models) → Neon `models` / `provider_model_variants` (see [model-catalog-ingestion.md](../reference/model-catalog-ingestion.md)). Carries `capabilities` (`ingestion_extraction`, `ingestion_validation`, `embedding`, `ingestion_embedding`…), `supportsStructuredOutput`, `supportsTools`, `modalities`, `editorialSummary`, `pricingRef`. This is what `GET /api/models` and the Models browser read.
...
|---|---|---|
| **0 — ship now** | None | `CatalogueRepository` interface over current Neon; derived-suitability + cost module; wire into general binding API/UI as advisory; embedding hard-guard + dimension-lock surfacing; Surreal dimension fix; capability/pricing QA backfill on the 143 seed models |
...
- **Suitability is only as good as the capability tags** — Phase 0 needs a QA pass over the 143 seed models before the guard goes hard.
...
| **Data / catalogue** | Capability-tag + pricing QA on the 143 seed models; aggregator family mapping; `$0`→"unknown" | Sonnet · medium (Haiku for rote) | `model-catalog-ingestion`, `catalog-governance` |
