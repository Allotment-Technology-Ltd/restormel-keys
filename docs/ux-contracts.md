...
| **Mobile read-only tier** (layout gate) | n/a | n/a | Off-tier path on a phone → the honest gate ("This screen needs a bigger window") naming the two readable destinations (Home, Claims) and the individual run (as plain text — `/runs` the list is itself gated, so it is **not** a link), no disabled-and-teasing actions | `/home`, `/runs/[id]`, `/claims` open read-only on a phone: sidebar hidden, full-bleed, ≥44px touch targets, mutating actions hidden. The layout's `.shell-mobile-readonly` rule hides `[data-mobile-hide]`, the run console action regions (`.run-actions` / `.run-cancel-wrap`), and — for the Claims explorer + readiness wizard — every mutation action region: `.review-actions`, `.dossier-actions`, `.dossier-recheck`, `.remove-section`, `.cohort-complete-actions`, `.revalidate-actions`, `.wizard-actions`. Read-only viewing (pan/zoom/select/inspect, glossary, provenance, recheck results) stays live. Guarded by `dashboard-mobile-readonly-claims.test.ts` (rule covers each selector ⇄ each selector still exists as a real action region). |
