...
**Published line:** `@restormel/testing-*` **v0.1.4+** on npm — use **`pnpm add -D @restormel/testing-cli@^0.1.4`** or the meta-package **`pnpm add -D @restormel/testing-bundle@^0.1.4`** (pulls CLI + browser adaptor dependency; still run Playwright browser install as below). Pin the same semver line across peer packages if you depend on them directly.
...
```bash
pnpm add -D @restormel/testing-cli@^0.1.4
