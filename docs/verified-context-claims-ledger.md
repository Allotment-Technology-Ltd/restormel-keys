...
| 7 | "Every claim carries a provenance trace" | Connect v1 retrieve responses carry, per claim: verification state, citation, trace ref; traces are exportable. | Trace builder/handler tests exist (`apps/dashboard/src/lib/server/connect-v1/provenance-trace-builder.test.ts`, `trace-handler.test.ts`); the per-claim envelope on the retrieve path is Stage 1.1's deliverable (in flight). | **partial** — owner: [Stage 1.1](verified-context-pivot-roadmap.md) |
