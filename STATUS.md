...
**Dogfooding / npm:** **validate** and **keys-cli** publish via `pnpm publish`. Free tier **2 projects**. **Founding Pro:** first **50** signups → **12 months Pro** ([docs/reference/founding-pro-promo.md](docs/reference/founding-pro-promo.md)). [docs/reference/npm-packages.md](docs/reference/npm-packages.md). **SOPHIA Phase 5A complete:** core integration through Phase 5 working with real KeyManager (from local tarball). Dogfood findings captured in [docs/reference/sophia-dogfood-findings.md](docs/reference/sophia-dogfood-findings.md) with prioritised next work (npm publish, async persistence, key-status model, server-side validation, provider expansion).

**Blockers:** UI packages (`@restormel/keys-svelte`, `@restormel/keys-react`, `@restormel/keys-elements`) return npm 404 — blocks Phase 5B for external consumers. See [npm-packages.md](docs/reference/npm-packages.md) and [sophia-dogfood-findings.md](docs/reference/sophia-dogfood-findings.md).
